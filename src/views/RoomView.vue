...
import { getIceServers, getSfuToken, joinCall } from '../api/calls.js';
...
async function bootstrapLiveKit(stream) {
  // Verify the user is signed in (either via Google or as a guest
  // session that PreJoinView set up silently). PreJoinView is the
  // entry point for non-authenticated users, so this should always
  // be true unless the user landed on /room/:id via a deep link.
  const me = await getMe();
  if (!me) {
    throw new Error('Necesitás iniciar sesión antes de entrar a la sala.');
  }
  // Register the current user as a call participant so the protected
  // endpoints below (sfu-token, turn-credentials) pass the
  // `isParticipant` check on the backend.
  await joinCall(props.roomId);
  if (!wsToken) {
